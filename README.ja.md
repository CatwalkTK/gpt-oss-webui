...
**Clavi Local Mining** は Next.js 15 と React 19 で構築された最新の ChatGPT クローンで、ローカル文書を活用できる **ベクトル検索RAG（Retrieval-Augmented Generation）** を備えています。
...
- **チャット内ドキュメント検索**: チャット画面でベクトル検索パネルを開き、その結果を回答に自動反映／必要に応じてインデックス利用を無効化
- **言語モード設定**: 設定画面から応答言語を選択可能
- **柔軟な環境設定**: テーマ、サイドバー位置、表示サイズをサイドバーから即時変更
...
### 文書検索 & RAG
1. **文書検索ツールを開く**:
   - メインチャットヘッダーの **Show Document Search** トグルをクリック、または
   - 専用ページ http://localhost:3000/search にアクセス
...
4. **文脈付きチャット**:
   - 一度インデックス化すれば、すべてのチャットメッセージで自動的に類似検索が走ります
   - メインチャットに滞在したままでも、該当部分がプロンプトへ注入されます
   - 可能な場合は応答に該当ドキュメントへの言及が含まれます

### 設定
1. サイドバー下部の **環境設定** を開く
2. 応答言語のほか、**テーマ**（ダーク/ライト）、サイドバーの**表示位置**、画面の**表示サイズ**を切り替え、必要に応じて **インデックス文書を利用** を無効化
3. ローカルストレージの使用量を確認したい場合は `/settings` ページでクリーンアップを実行
